...
@Injectable()
export class ExternalEventsSimpleService {
  private readonly logger = new Logger(ExternalEventsSimpleService.name);
  ...
  async getLocalEvents(location: string, radiusKm: number = 25): Promise<SimpleEvent[]> {
    try {
      if (!this.apiKey) {
  ...
  private processEvents(events: any[]): SimpleEvent[] {
    return events
  ...
  async getTransportationDisruptions(location: string): Promise<SimpleEvent[]> {
    try {
