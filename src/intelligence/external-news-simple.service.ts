...
@Injectable()
export class ExternalNewsSimpleService {
  private readonly logger = new Logger(ExternalNewsSimpleService.name);
  ...
  async getFoodRelatedNews(location: string): Promise<SimpleNewsData> {
    try {
      if (!this.apiKey) {
